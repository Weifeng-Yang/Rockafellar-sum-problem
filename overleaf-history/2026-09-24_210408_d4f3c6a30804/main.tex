...
% By further utilizing this general construction theorem, we provide families of counterexamples under the same interior-domain condition. More importantly, this general construction theorem provides a systematic mechanism for generating entire families of counterexamples under the same interior-domain condition. These families of counterexamples extend to Banach spaces containing a closed subspace or admitting a quotient isomorphic to $c_0$ or $\ell^1$, and include examples on $c_0$ with normal cones to closed balls of any prescribed positive radius, as well as examples with second operators having full domain and bounded range. 

% The construction theorem further yields families of counterexamples satisfying the same interior-domain condition. In particular, such counterexamples exist on every Banach space that either contains a closed subspace isomorphic to $c_0$ or $\ell^1$ or admits one of these spaces as a quotient. The construction also yields examples on $c_0$ with normal-cone second operators associated with closed balls of any prescribed positive radius, as well as examples whose second operators have full domain and bounded range.
...
% More significantly, this construction theorem provides a systematic mechanism for generating entire families of counterexamples, since the construction can be transferred to further Banach spaces and the second operator can be varied, while the interior-domain condition in Eq. (\ref{eq:original-cq}) and maximal monotonicity of both summands are preserved.


% Specifically, we develop a general construction theorem that computes the entire monotone polar of a class of graphs, characterizes their maximality, and identifies a positive rank-one perturbation whose addition makes the sum nonmaximal. To satisfy the theorem's assumptions on $c_0$, we couple copies of the triangular map through a Lipschitz curve, and then obtain a counterexample to the conjecture. A bounded linear surjection then transfers this counterexample to standard $\ell^1$. The corollaries further yield families of counterexamples by transferring these constructions to further Banach spaces and varying the second operator, while retaining Eq. (\ref{eq:original-cq}) and maximal monotonicity of both factors.
...
(3) We construct a counterexample on standard $\ell^1$ (Theorem~\ref{thm:L}) by pulling back the first operator through a specified bounded linear surjection onto $c_0$. By applying Lemma~\ref{lem:surjection-transport}, we prove that both operators are maximally monotone in $\ell^1\times\ell^\infty$. We also establish nonmaximality of their sum by showing that $(0,0)$ lies outside its graph and is monotonically related to every point of that graph.
...
\end{equation}
Thus $D$ consists of the dual variables whose coordinates
$(r(a),Ma)$ lie on the prescribed curve, and $G$ is obtained by
lifting each $a\in D$ to the point $(La,a)\in X\times X^*$.
...
In this section, we apply Theorem~\ref{thm:endpoint-criterion} and Lemma~\ref{lem:surjection-transport} to derive families of counterexamples and determine their structural properties. We first vary the block weights, affine values, second operator and underlying space. For the resulting first operators, we determine the domain closure, the size of the actual domain and the exact radial bound, and give a criterion for membership of zero in the actual convex hull. We then characterize reflexivity by a fixed rank-one test within two classical classes of spaces, identify all maximal extensions under pullback, and compute exact Fitzpatrick values and their pullback formulas. Further consequences are organized by topic in Appendices~\ref{app:parameters}--\ref{app:fitzpatrick}.
\subsection{Families from the construction theorem}
\label{subsec:counterexample-families-main}
We first vary the block weights and the curve, reducing maximality to exclusion of the curve's limit from an explicitly computed range. We then prescribe the affine values of the first operator and show that sum failure persists under positive rescaling and continuous monotone additions to the second operator. These families follow from Theorem~\ref{thm:endpoint-criterion}.
...
For the smooth choice, take $\chi\in C^\infty([0,\infty))$ with $0\le\chi\le1$, $\chi=1$ on $[0,1/2]$, and $\chi=0$ on $[1,\infty)$. Put
\begin{equation}
...
We next change the first operator's values by a skew modification of the map from dual variables to primal variables. The block masses and the parameter fibres remain those verified above.
...
We now keep the first operator fixed. The following proposition shows that every positive coefficient gives a nonmaximal sum and that adding any continuous monotone map to the second operator preserves this failure. Its proof also produces mutually incompatible polar points and hence distinct maximal extensions.
...
The preceding families vary operators on $c_0(I)$. We next vary the ambient space. The pullback lemma transfers the examples on $c_0$ and $\ell^1$ through surjections, and the Hahn--Banach theorem extends them from closed subspaces. The next proposition gives the resulting classes of spaces.
...
\end{proposition}
\begin{proof}
Let $V$ be $c_0$ or $\ell^1$, and denote the corresponding pair from Theorem~\ref{thm:C} or Theorem~\ref{thm:L} by $A_V,P_V$. In both cases,
\[
P_Vv=\pair{v}{g_V}g_V
\]
...
Let $Y$ be a closed subspace of $X$ and $U:Y\to V$ a Banach space isomorphism. By Lemma~\ref{lem:surjection-transport}, the operators
\[
...
Let $Q:X\to V$ be a bounded linear surjection. The open mapping theorem supplies the preimage bound required in Lemma~\ref{lem:surjection-transport}. Define
\[
A_X=Q^*A_VQ,\qquad P_X=Q^*P_VQ.
\]
That lemma proves maximality of both operators. Moreover,
\[
...
In a follow-up to our construction, Bo\c{t} \cite{bot2026fpv} gives a geometric-series realization with a nonconvex domain closure and a lower bound showing divergence of dual values at the boundary. Here we compute the entire domain closure and the estimate at its two boundary hyperplanes for the weighted families with prescribed affine values. We also show that the actual domain is meagre and determine its exact radial bound. We then give a separate criterion for membership of the origin in its actual convex hull.

The full parameter fibres also determine the norm closure of the domain. In the following statement, $W_B$ is the closed subspace of $Y$ annihilating $V_B$:
\begin{equation}
...
-\ell s\left(\sum_{n=1}^Nw_n^{-2}\right)^{1/2}
\qquad(N\ge1).
\end{aligned}
...
\subsubsection{The actual convex hull}
\label{subsec:appendix-convex-hull}

The closure in Eq. (\ref{eq:family-domain-closure}) is independent of the curve. Membership of zero in the actual convex hull has a separate criterion, which already follows from the abstract construction.
...
Lemma~\ref{lem:surjection-transport} preserves maximality, and the adjoint identity transfers the polar inequalities used to certify the counterexamples. We now compute the entire monotone polar of the pullback of any nonempty graph. For a monotone graph, this formula also identifies all maximally monotone extensions of its pullback.
...
\end{corollary}
\begin{proof}
Let $(u,u^*)\in(\mathcal L_QG)^\mu$. Fix $(x,a)\in G$ and a lift $v$ of $x$. Testing against $(v+tk,Q^*a)$ for $k\in\ker Q$ and both signs of $t$ forces $u^*$ to annihilate $\ker Q$. The factorization argument in Lemma~\ref{lem:surjection-transport} gives a unique $p\in X^*$ with $u^*=Q^*p$. The adjoint identity then gives $(Qu,p)\in G^\mu$. The converse follows from the same identity, and injectivity of $Q^*$ gives the set-difference formula.
...
E=\{(Qu,a):(u,Q^*a)\in H\}
\]
is monotone and contains $G$. Its pullback is monotone and contains $H$, so maximality gives $H=\mathcal L_QE$. A proper monotone extension of $E$ would pull back to a proper extension of $H$, hence $E$ is maximally monotone. Conversely, Lemma~\ref{lem:surjection-transport} lifts every maximal extension of $G$. Surjectivity of $Q$ and injectivity of $Q^*$ show that these assignments are inverse. Apply this bijection to Proposition~\ref{cor:positive-perturbations}(1).
\end{proof}
...
We next compare the Fitzpatrick function of a sum with the infimum obtained by splitting the dual variable between its two factors. The construction gives an exact positive difference even though the dual minimum is finite and uniquely attained. We then show that the same difference and the corresponding minimizers are preserved under surjective pullback.

\subsubsection{Exact values and perturbation strength}
...
\end{equation}
We compare the Fitzpatrick value of the sum at the origin with the minimum obtained by splitting the dual variable between its two factors.
...
\end{cases}
\]
Indeed, the supremum is $\sup_{x\in X}\{\pair{x}{p}-\lambda\pair{x}{g}^2\}$. A direction in $\ker g$ makes it infinite unless $p\in\R g$, and scalar maximization gives the displayed finite value. Consequently, the objective in Eq. (\ref{eq:cor-split-minimum}) is at least $S+|\beta|+\frac{\beta^2}{4\lambda}$ when $p=\beta g$, with value $S$ only at $\beta=0$. Finally, the defining polar inequality is equivalent to $\mathsf F_T(z,p)\le\pair{z}{p}$. Apply this equivalence at $(0,0)$ to obtain the threshold.
\end{proof}
...
\subsubsection{Fitzpatrick functions under surjective pullback}
\label{subsec:appendix-fitzpatrick-transfer}

We next show that the Fitzpatrick functions and their dual decompositions are preserved by surjective pullback.
...
In this paper, we constructed counterexamples to Rockafellar's sum conjecture, thereby providing the first complete disproof of the conjecture and resolving this long-standing problem after more than five decades. Specifically, we established a general construction theorem that computes the monotone polar of the proposed graphs, characterizes their maximal monotonicity and identifies when adding an everywhere-defined positive rank-one operator produces a nonmaximal sum. We verified its hypotheses for explicit operators on $c_0$ and transferred the resulting pair to standard $\ell^1$ through a bounded linear surjection. In both pairs, the two operators are maximally monotone and satisfy the original interior-domain condition, and their sum is not maximally monotone. We also proved a finite radial bound for the first operator in each pair.


The corollaries further yielded families of counterexamples on Banach spaces containing a closed subspace or admitting a quotient isomorphic to $c_0$ or $\ell^1$. On $c_0$, they gave examples with normal cones to closed balls of every prescribed positive radius. The radial bounds also gave families of second operators with full domain and bounded range whose sums with the constructed first operators are not maximally monotone. All these pairs satisfy the original interior-domain condition.

\section*{Code availability.}
The Lean formalization of this paper is available at \url{https://github.com/Weifeng-Yang/RockafellarSumConjecture}
...
\section*{Appendices}
These appendices develop further consequences of the same construction and pullback results. Appendix~\ref{app:parameters} concerns the first operators and their representations, Appendix~\ref{app:partners} varies the second operators, Appendix~\ref{app:pullback} treats transferred constraints and affine factors, and Appendix~\ref{app:fitzpatrick} relates radial bounds to Fitzpatrick values and finite graph averages.

\section{Further geometry of the first operators}
\label{app:parameters}

We supplement the domain and value formulas in the main text with additional parameter choices and consequences for Hilbert-space representations and local graph membership.

\subsection{Further parameter choices and their transfer}
\label{subsec:appendix-parameter-transfer}

We record further choices of the curve and block weights, and then describe which value and domain properties are preserved under pullback.

\begin{remark}[Further parameter choices]
Arbitrary signs of the coordinates of $\eta$ are allowed throughout, since the estimates use absolute values and squares. For example, in the unweighted case choose $\eta_n=\varepsilon_nc n^{-p}$, where $\varepsilon_n\in\{-1,1\}$, $1/2<p\le1$ and $c>0$ makes $\|\eta\|_2<1$. Choose $0<q<p-1/2$ and $\delta>0$ with $\delta\|(c n^{q-p})_n\|_2\le1$. Then $\omega_n(s)=\eta_n\max\{1-\delta s n^q,0\}$ is admissible. With $N=(\delta s)^{-1/q}$, its absolute sum is bounded above by $c\sum_{n\le N}n^{-p}$ and below by $(c/2)\sum_{n\le2^{-1/q}N}n^{-p}$. Consequently,
\[
\|\omega(s)\|_1\asymp
\begin{cases}
s^{-(1-p)/q},&p<1,\\
...
For the weighted families $A_{w,B,\omega}$, surjective pullback preserves the affine value dimensions and transfers the domain closure and actual-convex-hull criterion.

\begin{remark}[Pullback of the parameter families]
Let $U$ be a real Banach space and let $Q:U\to Y$ be a bounded linear surjection with the preimage constant $C_Q$ in Lemma~\ref{lem:surjection-transport}. For $A=A_{w,B,\omega}$, set $T=Q^*AQ$ and $P_U=Q^*PQ$, so $P_Uu=\pair{u}{Q^*g}Q^*g$. That lemma gives maximality of both operators, and surjectivity gives $\dom T\ne\varnothing$ and $Q^*g\ne0$. For $(Qu,a)\in\gra A$, Eq. (\ref{eq:abs-pairing}) yields
\[
\pair{u}{Q^*a+P_Uu}
=\pair{Qu}{a+PQu}
...
Bo\c{t} \cite{bot2026fpv} obtains failure of type~(FPV) from nonconvexity of the domain closure. The following result records an explicit local polar point and computes its finite global Fitzpatrick value.

We use the Fitzpatrick function defined in Eq. (\ref{eq:cor-fitzpatrick}).
...
Localizing the domain to a ball and then taking its convex hull need not give the same optimization value as convexifying first. This difference can be quantified by an affine change of either concrete example.
...
\]
Both signs are domain points because $\omega_1=0$. On $\ell^1$, choose the index $i$ with $q_i=x_2/8$ and instead use $x_2=8e_i$. Put $\varphi=k/2$. On $c_0(I)$ take $e=e_{0,1}-e_{0,2}$, and on $\ell^1$ take $e=e_j$ with $q_j=e_{0,1}-e_{0,2}$. The adjoint isometry in the construction gives $\|\varphi\|=\|e\|=\pair{e}{\varphi}=1$ in both spaces.

Set $r_0=3/4$ and $\mathcal Sz=-x_2\pair{z}{\varphi}/r_0+z-\pair{z}{\varphi}e$. Writing $z=\alpha e+w$, $w\in\ker\varphi$, shows that $\mathcal Sz=-\alpha x_2/r_0+w$ and $\pair{\mathcal Sz}{\varphi}=-\alpha/r_0$. These equations give a bounded inverse. Consequently $\widetilde M(z)=\mathcal S^*M(x_2+\mathcal Sz)$ is maximally monotone by the isomorphism case of Lemma~\ref{lem:surjection-transport} and translation.

Every $z\in\dom\widetilde M$ satisfies $|2-2\pair{z}{\varphi}/r_0|>1$, hence $\pair{z}{\varphi}<3/8$ or $\pair{z}{\varphi}>9/8$. The second case is impossible in the unit ball. The preimages of $x_2,-x_2$ are $0,3e/2$, so the first supremum has the stated bounds. Since $e\in\operatorname{conv}(\dom\widetilde M)$, for every $p\in X^*$,
\[
\sup_{z\in\dom\widetilde M}\pair{z}{p}+\|\varphi-p\|
\ge\pair{e}{p}+\pair{e}{\varphi-p}=1.
\]
The choice $p=0$ attains this bound. The same two alternatives give the distance estimate at $d=3e/4$. Since $x_2+\mathcal Sd=0$, the adjoint identity gives
\[
...
Proposition~\ref{cor:positive-perturbations} gives nonmaximal sums for all positive rank-one coefficients. We now replace the linear scalar dependence by a nonlinear one. The first result uses the origin as its missing polar point, and the second allows that point to vary so that arbitrarily small bounded partners are included.

The scalar dependence of the second operator can also be nonlinear and bounded, without changing the first operator.
...
Let $A$ and $P$ be the operators on $c_0(I)$ in Theorem~\ref{thm:C}. Choose $w_i>0$ for $i\in I$ with $\sum_{i\in I}w_i<\infty$, and let $\varepsilon>0$.
\begin{enumerate}
...
The same perturbation argument, combined with Lemma~\ref{lem:surjection-transport}, gives strict second operators on the space before taking a quotient. Strictness must be supplied on that space, since a nonzero quotient kernel would otherwise leave directions on which the pulled-back operator is constant.

\begin{proposition}[Strict partners on the source space]
\label{cor:source-strict-partners}
Suppose $U$ has a countable family $(f_n)\subset U^*$ separating points, with $\|f_n\|\le1$, and $Q:U\to c_0(I)$ is a bounded linear surjection. Let $A,P$ be the operators in Theorem~\ref{thm:C}, and put $A_Q=Q^*AQ$, $P_Q=Q^*PQ$. Choose $w_n>0$ with $\sum_nw_n<\infty$ and define
\[
...
\end{enumerate}
These conclusions apply to standard $\ell^1$, using its coordinate functionals and the specified quotient in Theorem~\ref{thm:L}.
\end{proposition}
\begin{proof}
...
The construction theorem also supplies second operators defined by convex constraints or convex penalties. We first use its pairing identity for normal cones, then add finite continuous convex functions and use radial or seminorm potentials. These choices include both multivalued partners and a bounded single-valued partner on standard $\ell^1$.

Bo\c{t} \cite{bot2026fpv} gives a halfspace normal-cone example for his geometric-series realization. We use the pairing identity of the construction theorem to obtain a criterion for convex constraints, which we then apply to closed balls. For a nonempty closed convex set $C\subset X$, define
\[
N_C(x)=
\begin{cases}
...
\end{cases}
\]
The following criterion applies to the abstract construction and gives standard closed-ball examples for the concrete operator on $c_0$.
...
\end{equation}
\item\label{cor:nc-ball} The pair $(A_r,B_r)$ for every $r>0$, where $A$ is the operator on $Y=c_0(I)$ from Theorem~\ref{thm:C}, $\overline B_Y=\{x\in Y:\|x\|_\infty\le1\}$, and
\begin{equation}
\begin{aligned}
...
Full-domain second operators can instead be obtained from convex functions of the norm. Unlike the bounded-range examples, the powers greater than one below give coercive second operators.
...
In Proposition~\ref{cor:nonsmooth-penalties}, one may take $f(x)=\rho\|x\|$ or the maximum of finitely many affine functions. These give nonsmooth, generally multivalued second operators. A separate variant uses a seminorm instead of the norm in Proposition~\ref{cor:radial-partners}.
...
On standard $\ell^1$, a separate construction replaces dependence on a single linear functional by a fixed coordinatewise second operator.

\begin{proposition}[A fixed coordinatewise partner on $\ell^1$]
\label{cor:fixed-coordinate-partner}
Let $T$ be the operator in Theorem~\ref{thm:L}, put $\widetilde T(x)=\frac12T(x/2)$, and index the coordinates of $\ell^1$ by $n\ge0$. Define
\[
...
The isomorphism $x\mapsto x/2$ in Lemma~\ref{lem:surjection-transport} gives maximality of $\widetilde T$. The bounds in Theorem~\ref{thm:L} yield $\|x\|_1>1$ and $\pair{x}{a}>-\sigma$ on its graph. Since $\sum_nw_n=1/2$ and $r^2/\sqrt{r^2+w^2}>|r|-w$ for $w>0$,
\[
...
Unlike the smooth partners above, $C_*$ is not norm-continuous as a map from $\ell^1$ to $\ell^\infty$: for $x^{(n)}=w_ne_n\to0$, $\|C_*x^{(n)}\|_\infty=\sqrt2$. Its maximality is supplied by its convex potential, not by a continuity argument.

The norm-subdifferential partner in Proposition~\ref{cor:radial-partners} with $p=1$ has bounded range. More generally, the finite radial bound alone gives this choice of partner, without the other structure of the construction theorem.
...
\]
Therefore, $(0,0)$ is monotonically related to $\gra(\mathcal T+J_\rho)$ and lies outside it. The sum is not maximally monotone, and the interior-domain condition holds because $J_\rho$ has full domain. For the first operators in Theorems~\ref{thm:C} and~\ref{thm:L}, one may take any $\rho\ge2\sigma$.
\end{remark}

\section{Further consequences of pullback}
\label{app:pullback}

The pullback lemma and the extension correspondence yield constrained examples, prescribed affine factors and consequences for specific classes of Banach spaces. We also compute a dual quotient norm for the specified surjection onto $c_0$.

\subsection{Convex constraints and affine factors}
\label{subsec:appendix-affine-transfer}

We first show how surjective pullback transports convex constraints and subdifferentials. We then prescribe affine factors by adjoining independent coordinates.

\begin{corollary}[Convex constraints under surjective pullback]
\label{cor:pullback-constraints}
Let $f:X\to\R\cup\{+\infty\}$ be proper, lower semicontinuous and convex. Then
\begin{equation}
...
\]
Direct substitution gives $S_0^*g_0=g$. With $A_g=S_0^*A_0S_0$, Lemma~\ref{lem:surjection-transport} gives maximality and transfers nonmaximality through
\[
...
The specified surjection onto $c_0(I)$ also records the size of its adjoint values in the quotient $\ell^\infty/c_0$. This gives information not determined by the primal norm and the pairing of a graph point.

\begin{corollary}[Unbounded dual quotient norms at fixed primal norm and pairing]
\label{cor:dual-quotient-recovered}
For the operator $T$ in Theorem~\ref{thm:L}, there are $(u_N,p_N)\in\gra T$ and a fixed $d\in\ell^1$ such that
\[
...
\section{Radial bounds and graph averages}
\label{app:fitzpatrick}

We first show that the specified quotient preserves the exact radial bound in Corollary~\ref{cor:family-domain-geometry}. We then distinguish a finite radial bound from a finite Fitzpatrick value and from domain membership, and give a finite averaging certificate for incompatible polar points of the sum.
...
\end{equation}
Indeed, replace $\frac12$ in the residual construction of Lemma~\ref{lem:specified-quotient} by any $0<\delta<1$. Density of the same dictionary gives an actual preimage with $\|u\|_1\le\frac{\|x\|_\infty}{1-\delta}$. The contraction bound gives the reverse inequality, and letting $\delta\downarrow0$ proves Eq. (\ref{eq:metric-quotient-norm}). No minimizing preimage is needed.
...
\]
For the upper bound, use $\|Qu\|_\infty\le\|u\|_1$ on every lifted graph point. For the reverse bound, fix a graph point with negative pairing and choose preimages with norms approaching $\|x\|_\infty$ in Eq. (\ref{eq:metric-quotient-norm}), then take the supremum over these graph points. In particular, the two operators in Theorems~\ref{thm:C} and~\ref{thm:L} have exact radial constant $2\sigma=\frac{\pi^2}{48}$.
...
\]
The last equality follows from the fixed-fibre approximation in Corollary~\ref{cor:family-domain-geometry}, rather than division of independently attained extrema.
\end{remark}
...
The exact radial bounds above accompany finite Fitzpatrick values at the origin. We now prescribe any positive radial bound and obtain an infinite Fitzpatrick value by adding a scalar coordinate and translating its dual value.
...
\end{proposition}
\begin{proof}
Let $T$ be the operator in Theorem~\ref{thm:L}, put $C=2\sigma$ and $\alpha=\frac{\theta}{1+C}$, and first work on $E=\ell^1\oplus_1\R$. Define
\[
...
\]
because $\pair{x}{j}=\rho\|x\|$ for every $j\in J_\rho(x)$. For the concrete operators, $\rho\ge2\sigma$ is exactly the condition for this origin witness and supplies a full-domain, bounded-range partner with a nonmaximal sum. These are explicit witnesses against the corresponding universal domain-membership, distance and stability assertions, rather than additional counterexample constructions.
\end{remark}

\subsection{Finite graph averages and incompatible polar points}
\label{subsec:appendix-graph-averages}

The nonuniqueness in Proposition~\ref{cor:positive-perturbations} also has a finite averaging certificate. It uses two actual points of the first graph and one point of the rank-one graph.
...
[q_0,q']=-\varepsilon,
\]
where $[(x,a),(y,b)]=\pair{x-y}{a-b}$. These certificates are preserved by the surjective pullback in Lemma~\ref{lem:surjection-transport}.
\end{corollary}
\begin{proof}
