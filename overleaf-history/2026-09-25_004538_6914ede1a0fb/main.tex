...
The paper is organized as follows. Section~\ref{sec:definitions} introduces the notation and basic definitions. Section~\ref{sec:endpoint-criterion} establishes the construction theorem and the pullback theorem. Section~\ref{sec:counterexamples} applies these results to construct counterexample families and explicit examples. Section~\ref{sec:corollaries} derives structural consequences concerning domain geometry, reflexivity in two classes of Banach spaces, maximal extensions and Fitzpatrick functions. Section~\ref{sec:conclusion} concludes the paper. Appendices~\ref{app:parameters}--\ref{app:fitzpatrick} give further geometric properties, choices of the second operator and consequences of pullback and additional counterexample families.  
...
\]
because $\pair{x}{j}=\rho\|x\|$ for every $j\in J_\rho(x)$. For the concrete operators, $\rho\ge2\sigma$ is exactly the condition for this origin witness and supplies a full-domain, bounded-range partner with a nonmaximal sum. These are explicit witnesses against the corresponding universal domain-membership and distance assertions, rather than additional counterexample constructions.
\end{remark}
