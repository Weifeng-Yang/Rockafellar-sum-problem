...
(3) We construct a counterexample on standard $\ell^1$ (Example~\ref{thm:L}) by pulling back the first operator through a specified bounded linear surjection onto $c_0$. By applying Theorem~\ref{lem:surjection-transport}, we prove that both operators are maximally monotone in $\ell^1\times\ell^\infty$. We also establish nonmaximality of their sum by showing that $(0,0)$ lies outside its graph and is monotonically related to every point of that graph.

(4) 

The paper is organized as follows. Section~\ref{sec:definitions} introduces the notation and basic definitions. Section~\ref{sec:endpoint-criterion} proves the structural theorem and the pullback theorem. Section~\ref{sec:counterexamples} constructs the counterexamples on $c_0$ and standard $\ell^1$ and derives their stated consequences. Section~\ref{sec:conclusion} concludes the paper. 
...
In this section, we establish a general construction theorem for maximally monotone operators whose sums with a positive rank-one operator with full domain are not maximally monotone. We also prove a pullback theorem showing that maximal monotonicity is preserved under pullback by a bounded linear surjection.  
...
First, to establish the monotonicity properties needed to compute $G^\mu$, we prove that $\widehat F$ is $1$-Lipschitz on $\R$. 
...
Now, we rewrite the condition $(x,p)\in G^\mu$ in terms of the scalar parameter $t$. 
...
The construction theorem produces a counterexample on a space where its hypotheses can be realized. Next, we show how to transfer a pair of operators through a bounded linear surjection $Q:U\to V$. The pullback $Q^*\mathcal M Q$ includes every preimage under $Q$, and the adjoint preserves the duality pairing. The following theorem uses these facts to preserve maximal monotonicity and transfer a point witnessing nonmaximality of a sum. The preimage bound in its statement is available for every bounded linear surjection by the open mapping theorem.
...
In this section, we apply the construction theorem to obtain families with different block weights, curves and affine values, and show that their sums remain nonmaximal for every positive rank-one coefficient and under continuous monotone additions. Next, we give explicit counterexamples on $c_0$ and standard $\ell^1$. These examples, together with the pullback theorem and extension from closed subspaces, yield counterexamples on further Banach spaces.
...
First, we vary the block weights and the curve, reducing maximality to exclusion of the curve's limit from an explicitly computed range. Then, we prescribe the affine values of the first operator and show that sum failure persists under positive rescaling and continuous monotone additions to the second operator. These families follow from Theorem~\ref{thm:endpoint-criterion}.
...
Next, we change the first operator's values by a skew modification of the map from dual variables to primal variables. The block masses and the parameter fibres remain those verified above.
...
\end{equation}
\end{definition}
The next lemma provides the bounded preimages needed in Theorem~\ref{lem:surjection-transport}, and no bounded linear right inverse of $Q$ is required.
...
The preceding families vary operators on $c_0(I)$. Next, we vary the ambient space. The pullback theorem transfers the examples on $c_0$ and $\ell^1$ through surjections, and the Hahn--Banach theorem extends them from closed subspaces. The next proposition gives the resulting classes of spaces.
...
The following result records an explicit local polar point and computes its finite global Fitzpatrick value.
...
The same perturbation argument, combined with Theorem~\ref{lem:surjection-transport}, gives strict second operators on the space before taking a quotient. Strictness must be supplied on that space, since a nonzero quotient kernel would otherwise leave directions on which the pulled-back operator is constant.
...
The isomorphism $x\mapsto x/2$ in Theorem~\ref{lem:surjection-transport} gives maximality of $\widetilde T$. The bounds in Example~\ref{thm:L} yield $\|x\|_1>1$ and $\pair{x}{a}>-\sigma$ on its graph. Since $\sum_nw_n=1/2$ and $r^2/\sqrt{r^2+w^2}>|r|-w$ for $w>0$,
\[
...
First, we show how surjective pullback transports convex constraints and subdifferentials. Then, we prescribe affine factors by adjoining independent coordinates.
...
The exact radial bounds above accompany finite Fitzpatrick values at the origin. Now, we prescribe any positive radial bound and obtain an infinite Fitzpatrick value by adding a scalar coordinate and translating its dual value.
