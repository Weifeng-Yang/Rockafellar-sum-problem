...
The preceding lemmas establish all the hypotheses of Theorem~\ref{thm:endpoint-criterion}, including Eq. (\ref{eq:abs-endpoint-exclusion}). Therefore, applying that theorem to the operators in Eq. (\ref{eq:D8}) gives the following counterexample, with the quantitative bounds stated below.
...
The Lean formalization of the counterexample on $c_0$ and the general pullback lemma, together with the pinned dependencies and build instructions, is available at \url{https://github.com/Weifeng-Yang/RockafellarSumConjecture}. 
