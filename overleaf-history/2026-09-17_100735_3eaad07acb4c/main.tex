...
In this paper, we give a general theorem for constructing an operation to obtain the first counterexamples to Rockafellar's sum conjecture, thereby resolving the problem after more than five decades. 
...
Monotone polars and convex representations are studied in \cite{martinezsvaiter2005,bauschkemclarensendov2006}, which can be express as follows. 
...
\end{definition}
For $G=\gra T$ as in the preceding definition, the quantity of Verona and Verona \cite{verona2008} satisfies
\[
\begin{aligned}
L(0,0,T)
...
\end{aligned}
\]
Multiplying the two inequalities by the nonnegative weights $\frac{1+\tau}{2}$ and $\frac{1-\tau}{2}$, respectively, and adding gives
\[
(b-\tau)^2+\|z-\eta\|^2+\nu^2\le0.
\]
...
<2\sigma.
\]
Hence $\pair{La}{a}\ge-2\sigma\|La\|_\infty$ for every $a\in D$, and taking the supremum gives $\mathcal V(\gra A)\le2\sigma<\frac{1}{4}$.
