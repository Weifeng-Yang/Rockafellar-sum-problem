...
For an operator $\mathcal T:X\rightrightarrows X^*$ with nonempty graph, its Fitzpatrick function is
\begin{equation}
...
\end{equation}
The equality at zero follows also from $W(s)\le S$.
...
Let $\mathcal A$ be as in Theorem~\ref{thm:endpoint-criterion}, with Eq. (\ref{eq:abs-endpoint-exclusion}) satisfied.
\begin{enumerate}
\item\label{cor:nc-general} Suppose a closed convex set $C\subset X$ satisfies
\begin{equation}
...
\end{equation}
Then $\mathcal A$ and $N_C$ are maximally monotone and satisfy the interior-domain condition, but their sum is not maximally monotone. A point witnessing this failure is $(0,-Sg)$.
\item\label{cor:nc-ball} Let $A$ be the operator on $Y=c_0(I)$ from Theorem~\ref{thm:C}, and let $\overline B_Y=\{x\in Y:\|x\|_\infty\le1\}$. Put
\[
c=\frac{15}{4}(e_{0,1}-e_{0,2}).
\]
For every $r>0$, define
\begin{equation}
A_r(u)=\frac4r A\left(\frac4r u+c\right),
\qquad B_r=N_{r\overline B_Y}.
\label{eq:cor-ball-operators}
\end{equation}
Both operators are maximally monotone and satisfy
\[
\dom A_r\cap\operatorname{int}(\dom B_r)\ne\varnothing.
\]
Their sum is not maximally monotone. The point $(z_r,p_r)$ defined by
\begin{equation}
z_r=-\frac{15r}{16}(e_{0,1}-e_{0,2}),
\qquad p_r=-\frac{4\sigma}{r}g
\label{eq:cor-ball-witness}
\end{equation}
is monotonically related to $\gra(A_r+B_r)$ and does not belong to it.
\end{enumerate}
\end{corollary}
\begin{proof}
\noindent\textit{\textup{(\ref{cor:nc-general})}}
The operator $N_C$ is maximally monotone by the maximality theorem for convex subdifferentials \cite{rockafellar1970subdiff}, and $\dom N_C=C$. Thus Eq. (\ref{eq:cor-nc-condition}) is the required interior-domain condition. Let $x=La\in C$ with $a\in D$, and let $n\in N_C(x)$. Put $t=r(a)=\pair{x}{g}$. Since $|t|>1$ and $t\ge-1$, we have $t>1$. Also, $0\in C$ implies $\pair{x}{n}\ge0$. Therefore, Eq. (\ref{eq:abs-pairing}) gives
\[
\begin{aligned}
\pair{-x}{-Sg-a-n}
&=St+\pair{x}{a}+\pair{x}{n}\\
&\ge t^2-1-S+St\\
&=(t-1)(t+1+S)>0.
\end{aligned}
\]
Hence $(0,-Sg)$ is monotonically related to $\gra(\mathcal A+N_C)$. It lies outside that graph because $0\notin\dom\mathcal A$.

\noindent\textit{\textup{(\ref{cor:nc-ball})}}
In the concrete construction, $\omega_1=0$. Define
\[
a^\star=5e_{0,1}-7e_{0,2}+3e_{0,3}.
\]
Eqs. (\ref{eq:D4})--(\ref{eq:D7}) give
\[
\begin{aligned}
r(a^\star)&=2,\\
ma^\star&=(1,0,0,\ldots)=F(2),\\
x^\star:=La^\star&=e_{0,1}-e_{0,2}-3e_{0,3}.
\end{aligned}
\]
Thus $a^\star\in D$ and $x^\star\in\dom A$. Set $C=c+4\overline B_Y$. Since $g=e_{0,1}-e_{0,2}$ and $\|g\|_1=2$,
\[
\begin{aligned}
\|c\|_\infty&=\frac{15}{4}<4,\\
\|x^\star-c\|_\infty&=3<4,\\
\inf_{x\in C}\pair{x}{g}
&=\pair{c}{g}-4\|g\|_1=-\frac12.
\end{aligned}
\]
Consequently, $0\in C$, $x^\star\in\dom A\cap\operatorname{int}C$, and $C$ satisfies Eq. (\ref{eq:cor-nc-condition}). Part~\textup{(\ref{cor:nc-general})} applies with $S=\sigma$.

Fix $r>0$ and set $\alpha=\frac4r$. The invertible transformation
\[
(x,a)\longmapsto\left(\frac{x-c}{\alpha},\alpha a\right)
\]
preserves pairings of differences, since
\[
\left\langle
\frac{x-c}{\alpha}-\frac{y-c}{\alpha},
\alpha a-\alpha b
\right\rangle
=\pair{x-y}{a-b}.
\]
It therefore preserves monotonicity and maximality. It transforms $A$ into $A_r$ and $N_C$ into $B_r$. Indeed, writing $x=\alpha u+c$, the normal inequalities for $n\in N_C(x)$ are exactly those for $\alpha n\in N_{r\overline B_Y}(u)$.

The point
\[
u_r^\star=\frac{x^\star-c}{\alpha}
\]
belongs to $\dom A_r$ and satisfies $\|u_r^\star\|_\infty=\frac{3r}{4}<r$, proving the interior-domain condition. The same transformation sends the witness $(0,-\sigma g)$ to $(z_r,p_r)$ in Eq. (\ref{eq:cor-ball-witness}) and preserves its pairings with every sum graph point. Finally, $\alpha z_r+c=0$ and $A(0)=\varnothing$, so $A_r(z_r)=\varnothing$. Thus the transformed witness lies outside the sum graph.
\end{proof}
