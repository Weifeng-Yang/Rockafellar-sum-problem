...
We disprove Rockafellar's sum conjecture by constructing maximally
monotone operators whose sum is not maximally monotone despite satisfying
the interior-domain condition. We give counterexamples on $c_0$ and
standard $\ell^1$, with one summand a bounded positive rank-one operator
defined on the whole space. We first compute the entire monotone polar
of a class of graphs and characterize their maximal monotonicity by the
solvability of explicit equations in the original continuous dual.
For the construction on $c_0$, we couple triangular operators through a
Lipschitz curve of block sums. The positive-index coordinates of each
curve value have finite support, and their common limit at the two
deleted endpoints belongs to $\ell^2\setminus\ell^1$. The polar formula
shows that any additional polar point would require these nonsummable
coordinates in the continuous dual, proving maximality. An exact pairing
identity then proves nonmaximality of the sum and a finite radial bound
for the first operator. A bounded surjection transfers the construction
to standard $\ell^1$, where maximality is proved in the full
$\ell^1\times\ell^\infty$ pair. 
...
monotone-polar closures. Our structural theorem concerns graphs containing
...
\end{equation}
where $p$ is the dual component of the polar point,
$m_p=(\sum_j p_{b,j})_{b\ge0}\in\ell^1(\Nzero)$ is its sequence of
block sums, and $\tau,\nu\in\R$. If $|\tau|>1$, the test $t=\tau$ returns a graph
...
(1) We give a counterexample to Rockafellar's sum conjecture on $c_0$
(Example~\ref{thm:C}). The second operator is bounded, positive, rank-one
and defined everywhere. The first operator has nonempty domain, so the
original interior-domain condition holds. We prove maximality of both
operators and exhibit a point monotonically related to their sum that
does not belong to its graph.

(2) We establish a formula for the entire monotone polar of the graph
class in Eq.~\eqref{eq:abs-full-graph} and a necessary and sufficient
condition for maximality (Theorem~\ref{thm:endpoint-criterion}). The proof
separates the roles of the pairing identity, the complete affine fibres
and the original continuous dual. It also yields a finite radial bound
and the rank-one perturbation that makes the sum nonmaximal. We verify
all hypotheses in the coordinate construction and derive nonconvexity
of the first operator's domain closure (Corollary~\ref{prop:domain-gap}).

(3) We construct a counterexample on standard $\ell^1$
(Example~\ref{thm:L}) by pulling back the first operator through a
specified bounded surjection onto $c_0$. Tests along the kernel force
every polar dual component to factor through the surjection, proving
maximality against the entire $\ell^\infty$ dual. This transfers the same
failure mechanism to a second Banach space.
...
For a monotone $G$, $G=G^\mu$ is equivalent to maximality: a point in
$G^\mu\setminus G$ can be adjoined to $G$, and every point of a monotone
extension belongs to $G^\mu$. We write
...
$Ma^t=F(t)$. We include every $a\in X^*$ with these values, defining
\begin{equation}
...
pairing, giving a positive bracket with the missing origin at every point
of the sum graph. A limit of profiles in $H$ contributes a polar point only when an
...
\begin{remark}[Comparison with domain-closure statements]
...
a Banach space is convex. Corollary~\ref{prop:domain-gap}, together
...
\begin{remark}[A bidual domination assertion]
...
bound $\|\mathcal R(x)\|_1\le2\|x\|_\infty$, and Example~\ref{thm:C} proves maximality
...
bracket of any two graph points $(u,Q^*a)$ and $(v,Q^*c)$ equals
...
$\ell^\infty(\N)$. Fix $(u,Q^*a)\in\gra T$. Since
$Q(u+\theta k)=Qu$ for $k\in\ker Q$ and $\theta\in\R$,
every $(u+\theta k,Q^*a)$ also belongs to $\gra T$. Testing these points forces
...
$(v,v^*)=(v,Q^*p)$ belongs to $\gra T$. This proves maximality in the
...
coordinate construction, their solutions over the deleted interval
