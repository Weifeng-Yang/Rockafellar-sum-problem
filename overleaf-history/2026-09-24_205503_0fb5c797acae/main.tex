...
In this section, we derive further consequences of the general construction theorem. Specifically, we compute exact Fitzpatrick values for the original pair and obtain families of counterexamples by varying the perturbation strength, the second operator and the underlying space. All these pairs satisfy the interior-domain condition in Eq. (\ref{eq:original-cq}).

First, for an operator $\mathcal T:X\rightrightarrows X^*$ with nonempty graph, its Fitzpatrick function is defined as
\begin{equation}
...
\end{equation}
We first compare the Fitzpatrick value of the sum at the origin with the minimum obtained by splitting the dual variable between its two factors.

\begin{corollary}[Fitzpatrick values at the origin]
\label{cor:fitzpatrick-values}
Let $\mathcal A$ and $\mathcal P$ be the operators in Theorem~\ref{thm:endpoint-criterion}, and suppose Eq. (\ref{eq:abs-endpoint-exclusion}) holds. Then
\begin{equation}
\mathsf F_{\mathcal A+\mathcal P}(0,0)=S-1
\label{eq:cor-sum-fitzpatrick}
\end{equation}
and
\begin{equation}
\min_{p\in X^*}
\{\mathsf F_{\mathcal A}(0,-p)+\mathsf F_{\mathcal P}(0,p)\}=S.
\label{eq:cor-split-minimum}
\end{equation}
The minimum in Eq. (\ref{eq:cor-split-minimum}) is attained only at $p=0$.
\end{corollary}
\begin{proof}
Set $W(s)=\|\omega(s)\|_{H_0}^2$ for $s>0$. Assumption~\ref{ass:construction} gives $W(s)\le S$ and $W(s)\to S$ as $s\downarrow0$. Every $t\in J$ is realized by an element of $D$. Therefore, Eq. (\ref{eq:abs-pairing}) gives
\[
\mathsf F_{\mathcal A+\mathcal P}(0,0)
=\sup_{|t|>1}\{1+W(|t|-1)-2t^2\}.
\]
The expression in braces is at most $S-1$ and tends to $S-1$ as $t\to1^+$. This proves Eq. (\ref{eq:cor-sum-fitzpatrick}).

For $\alpha\in\R$, define $H(\alpha)=\mathsf F_{\mathcal A}(0,\alpha g)$. The same pairing identity gives
\[
H(\alpha)=\sup_{|t|>1}
\{\alpha t+1+W(|t|-1)-t^2\}.
\]
Taking $t\to1^+$ and $t\to-1^-$, respectively, yields
\begin{equation}
H(\alpha)\ge S+|\alpha|,
\qquad H(0)=S.
\label{eq:cor-fitzpatrick-line}
\end{equation}
The equality at zero also follows from $W(s)\le S$.

Since $\mathcal P x=\pair{x}{g}g$, we have
\[
\mathsf F_{\mathcal P}(0,p)
=\sup_{x\in X}\{\pair{x}{p}-\pair{x}{g}^2\}.
\]
If $p\notin\R g$, there is $v\in\ker g$ with $\pair{v}{p}\ne0$. Testing $x=tv$ for $t\in\R$ makes this supremum infinite. If $p=\beta g$, the nonzero functional $g$ takes every real value, so
\[
\mathsf F_{\mathcal P}(0,\beta g)
=\sup_{t\in\R}\{\beta t-t^2\}
=\frac{\beta^2}{4}.
\]
Consequently, Eq. (\ref{eq:cor-fitzpatrick-line}) gives
\[
\mathsf F_{\mathcal A}(0,-\beta g)
+\mathsf F_{\mathcal P}(0,\beta g)
\ge S+|\beta|+\frac{\beta^2}{4}.
\]
Equality with $S$ holds at $\beta=0$ and is impossible for $\beta\ne0$. Points outside $\R g$ give an infinite value, proving Eq. (\ref{eq:cor-split-minimum}) and uniqueness.
\end{proof}

Thus the splitting minimum at the origin is finite and uniquely attained for these operators, whose sum is not maximally monotone by Theorem~\ref{thm:endpoint-criterion}.

\subsection{Further families of counterexamples}
\label{subsec:counterexample-families}
The construction theorem gives more than the two examples above. We show that the same first operator has a nonmaximal sum with every positive multiple of the rank-one operator, and that this failure persists under continuous monotone additions to the second operator. This also gives strictly monotone second operators of infinite rank. We then vary the scalar function, transfer the examples to further spaces, and construct second operators arising from convex constraints and radial convex functions. Every pair below consists of maximally monotone operators satisfying Eq. (\ref{eq:original-cq}).

\begin{corollary}[Positive coefficients and monotone perturbations]
...
\begin{corollary}[Scalar second operators]
...
\end{corollary}
\begin{proof}
The scalar monotonicity of $f$ makes $B_f$ continuous and monotone, so the direct polar argument in Corollary~\ref{cor:positive-perturbations}(2) proves its maximality. For $(x,a)=(La,a)\in\gra\mathcal A$, put $t=r(a)$ and $W=\|\omega(|t|-1)\|_{H_0}^2\le S$. Since $|t|>1$, the assumptions on $f$ give $tf(t)\ge S|t|$. Therefore,
\[
...
Strict monotonicity of the second operator does not remove the failure. The next corollary replaces the rank-one partner by injective compact linear operators of infinite rank, and also gives smooth strictly monotone second operators with bounded range.

\begin{corollary}[Strictly monotone second operators]
...
\]
Moreover, the tails of the coordinate sums are uniformly bounded by the tails of $\kappa|g_i|+\varepsilon w_i$. Finite-coordinate truncation therefore proves relative compactness of the whole range. The bounds on the derivatives of $\tanh$, together with $\sum_iw_i<\infty$, justify differentiation of the series to every order. Thus the displayed potential is smooth, has derivative $\widetilde B_{\kappa,\varepsilon}$ and is strictly convex. The direct polar argument for continuous monotone maps gives maximality. Corollary~\ref{cor:scalar-partners}, with $S=\sigma$, gives $\pair{x}{a+\kappa\tanh(\pair{x}{g})g}>0$ on $\gra A$. Adding $\varepsilon\sum_iw_ix_i\tanh x_i\ge0$ preserves this inequality, so $(0,0)$ remains a point in the monotone polar outside the sum graph. Full domain gives Eq. (\ref{eq:original-cq}).
\end{proof}

The counterexamples on $c_0$ and $\ell^1$ also yield counterexamples on spaces that contain either of them as a closed subspace, up to isomorphism, or admit either as a quotient.

\begin{corollary}[Transfer to further Banach spaces]
...
We next use the pairing identity to replace the rank-one second operator by a normal-cone operator. For a nonempty closed convex set $C\subset X$, define
\[
N_C(x)=
\begin{cases}
...
\begin{corollary}[Normal cones and closed balls]
...
The normal-cone examples also permit a smooth convex penalty on the constraint set.

\begin{corollary}[Convex constraints with smooth penalties]
\label{cor:constrained-penalties}
Let $\mathcal A,C$ satisfy the hypotheses of Corollary~\ref{cor:normal-cone}\textup{(\ref{cor:nc-general})}, and let $f:X\to\R$ be convex and continuously Fr\'{e}chet differentiable. Write $\iota_C=0$ on $C$ and $\iota_C=+\infty$ outside $C$. Then
\[
B_f=\partial(\iota_C+f)=N_C+Df
\]
is maximally monotone with domain $C$, and $\mathcal A+B_f$ is not maximally monotone under Eq. (\ref{eq:original-cq}). A point in its monotone polar outside its graph is $(0,-Sg+Df(0))$. The same conclusion applies to each closed-ball pair in Corollary~\ref{cor:normal-cone}\textup{(\ref{cor:nc-ball})}, with witness $(z_r,p_r+Df(z_r))$.
\end{corollary}
\begin{proof}
For $x\in C$ and $b\in\partial(\iota_C+f)(x)$, test the subgradient inequality at $x+t(y-x)$, where $y\in C$ and $t\downarrow0$. It gives $\pair{y-x}{b-Df(x)}\le0$, so $b-Df(x)\in N_C(x)$. The converse follows by adding the subgradient inequality for $f$ to the normal-cone inequality. This proves the stated equality, including the empty values outside $C$. Since $0\in N_C(x)$ for all $x\in C$, its domain is $C$. The function $\iota_C+f$ is proper, lower semicontinuous and convex, so $B_f$ is maximally monotone \cite{rockafellar1970subdiff}.

For $a\in\mathcal A(x)$ and $n\in N_C(x)$, Corollary~\ref{cor:normal-cone} and the monotonicity of $Df$ give
\[
\pair{-x}{-Sg+Df(0)-a-n-Df(x)}
=\pair{-x}{-Sg-a-n}+\pair{x}{Df(x)-Df(0)}\ge0.
\]
The first operator has no value at zero, and the common-domain condition is unchanged. For the closed-ball pairs, the identical calculation at $z_r$ uses the witness in Eq. (\ref{eq:cor-ball-witness}).
\end{proof}
...
\begin{corollary}[Radial convex functions]
...
The case $p=1$ has bounded range. More generally, the finite radial bound alone gives this choice of partner, without the other structure of the construction theorem.
