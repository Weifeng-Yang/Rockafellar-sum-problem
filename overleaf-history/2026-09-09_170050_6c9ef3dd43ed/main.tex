...
\end{equation}
The unrestricted sum question asks whether Eq. (\ref{eq:original-cq}) alone is sufficient on an arbitrary real Banach space. The interior here is taken in the norm topology of $X$, and maximality is taken in the full pair $X\times X^*$.

In general Banach spaces, sum theorems have been established under additional assumptions on the operators. Yao \cite{yao2010} proves maximality when $A$ is of type~(FPV) and $B$ has full domain. Borwein and Yao \cite{borweinyao2012} prove maximality under Eq. (\ref{eq:original-cq}) when $A$ is a linear relation. Voisei \cite{voisei2010} treats the sum of an operator of type~(FPV) with convex domain and a normal-cone operator. Verona and Verona \cite[Main Result~1.1(b)]{veronaverona2022} prove the sum theorem under Eq. (\ref{eq:original-cq}) when $A$ is $\mathcal C_0$-maximal: $A+N_C$ is maximally monotone for every closed convex set $C$ with $\dom A\cap\operatorname{int}C\ne\varnothing$, where $N_C$ denotes the normal-cone operator of $C$. Bo\c{t}, Bueno, and Simons \cite{botbuenosimons2016} show that the unrestricted sum question is equivalent to its restriction to pairs with bounded common domain.

Convex representations relate monotone operators to convex functions. Fitzpatrick \cite{fitzpatrick1988} associates a convex function with a monotone graph, and Burachik and Svaiter \cite{burachiksvaiter2002} study the associated family of convex representatives through enlargements. These representations also yield results about sums: Penot \cite{penot2004} develops representation methods for compositions and sums in reflexive spaces, and Marques Alves and Svaiter \cite{marquessvaiter2012} prove a sum theorem in general Banach spaces under a qualification on the domains of Fitzpatrick representations.

Since Rockafellar's theorem covers reflexive spaces, a counterexample must be constructed in a nonreflexive space. Bueno and Svaiter \cite{buenosvaiter2011} construct a maximally monotone operator on $c_0$. Bauschke, Borwein, Wang, and Yao \cite{bauschkeetal2011} develop triangular operators and nonlinear examples on nonreflexive Banach spaces. Their triangular map is the building block used in our construction in Section~\ref{sec:counterexamples}.

In this paper, we construct counterexamples to Rockafellar's sum conjecture on $c_0$ and standard $\ell^1$. Our general construction theorem computes the entire monotone polar of a class of graphs, characterizes their maximality, and identifies a positive rank-one perturbation whose addition makes the sum nonmaximal. To satisfy the theorem's assumptions on $c_0$, we couple copies of the triangular map through a Lipschitz curve. A bounded linear surjection then transfers this counterexample to standard $\ell^1$.
...
(3) We construct a counterexample on standard $\ell^1$ (Example~\ref{thm:L}) by pulling back the first operator through a specified bounded linear surjection onto $c_0$. The pullback lemma proves maximality in $\ell^1\times\ell^\infty$, and the duality pairing preserves the inequalities that establish nonmaximality of the sum.
...
\end{equation}
\end{definition}
For $G=\gra T$, Eq. (\ref{eq:D3}) gives $\mathcal V(G)=L(0,0,T)$ in the notation of Verona and Verona \cite[p.~1004]{verona2008}. When finite, $\mathcal V(G)$ is the least $C\ge0$ such that $\pair{x}{a}\ge-C\|x\|$ for every $(x,a)\in G$. We derive explicit upper bounds for the two constructed graphs below.
...
Let $X$ be a real Banach space and let $H_0$ be a real Hilbert space. Define $H=\R\oplus H_0$ with the Hilbert direct-sum inner product
\[
...
\end{aligned}
\]
Here $\sgn t=t/|t|$ for $t\ne0$. The admissible dual variables and the graph are
\begin{equation}
\begin{aligned}
...
\end{align*}
Replacing $a$ by $a+\theta k$, for all $\theta\in\R$ and $k\in K$, forces $\pair{x+Ep}{k}=0$. Hence $x=-Ep+v h$ by Eq. (\ref{eq:abs-annihilator}). With
\[
...
We have proved that $G^\mu$ is monotone. Any point related to it is related to $G$ and therefore belongs to $G^\mu$, so $G^\mu$ is maximally monotone. Every monotone extension of $G$ lies in $G^\mu$; hence a maximal one must equal it.
...
\]
is maximally monotone in the full pair $U\times U^*$. Here $Q^*:V^*\to U^*$ is the adjoint of $Q$, defined by $\pair{u}{Q^*a}=\pair{Qu}{a}$ for $u\in U$ and $a\in V^*$.
\end{lemma}
\begin{proof}
...
\]
Maximality of $\mathcal M$ gives $(Qv,p)\in\gra\mathcal M$, and hence $(v,v^*)$ belongs to the pullback graph.
\end{proof}
...
\begin{proof}
\label{sec:C2}
For each $s>0$, $\omega_s(n)=0$ whenever $n\ge s^{-4}$. Also $0\le\omega_s(n)\le1/(4n)$. Hence $W(s)\le\sigma<1/8$, and dominated convergence in $\ell^2$ gives $\omega_s\to\omega_0$ and $W(s)\to\sigma$ as $s$ decreases to zero. The comparison vector $\omega_0$ is not in $\ell^1$ by divergence of the harmonic series.
...
\section*{Use of AI}
The author supplied earlier constructions and results, involving the sign function and triangular operators, together with obstruction results and a framework for positive rank-one perturbations, and guided their further development to investigate how maximality can fail under addition. Through iterative discussions of these materials, the author and GPT-5.6 Sol developed a general construction theorem characterizing maximality and identifying when an everywhere-defined rank-one monotone perturbation yields a nonmaximal sum.  Under the author's direction, GPT-5.6 Sol worked out the detailed constructions and proof arguments, including the coupling of triangular operators with a Lipschitz curve whose limits at $t=-1$ and $t=1$ lie outside the range of the block-sum map on $(c_0)^*$, and the transfer from $c_0$ to standard $\ell^1$.
